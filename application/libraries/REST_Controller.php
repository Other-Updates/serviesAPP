...
//        $access_token = $this->input->get_request_header('Accesstoken', TRUE);


        $this->_check_login_access($username, $password);
        //$this->_force_login();
    }
    ...
    private function _check_login_access($username = '', $password = '') {
        ...
        if (!empty($username) && !empty($password)) {
            ...
            if ($username == $verify_username && $password == $verify_password) {
                return true;
            } else {
                $output = array('status' => array('error' => 'true', 'code' => '401', 'type' => 'Unauthorized', 'message' => 'Authentication Failure'));
    ...
    public function _get_custom_post_values() {
        $json_input = json_decode(file_get_contents('php://input'), TRUE);
        if (empty($json_input)) {
            $get_arr = is_array($this->ci->input->get(null)) ? $this->ci->input->get(null) : array();
            $post_arr = is_array($this->ci->input->post(null)) ? $this->ci->input->post(null) : array();
