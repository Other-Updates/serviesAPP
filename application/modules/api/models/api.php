...
class Api extends REST_Controller {
    ...
    function customer_mail($insert) {
        ...
        $headers .= "Reply-To: Incredible Solutions <otp.incredible@gmail.com>\r\n";
        $headers .= "Return-Path: Incredible Solutions <otp.incredible@gmail.com>\r\n";
        $headers .= "From: Incredible Solutions <otp.incredible@gmail.com>\r\n";
        ...
        if(mail($email_id, "Incredible Solutions - OTP Password Generation", $html_message, $headers))
    ...
    function customer_gmail_post() {
        ...
        $message .= 'Greetings! One Time Password is generated for your account (USER NAME: ' . ucfirst($data['customer'][0]['name']) . ') at Incredible Solutions. Your OTP password is: <b>' . $data['customer'][0]['otp_pincode'] . '</b>. Thank You!<br/>';
        $message .= '<br/><br/>Regards,<br/> Incredible Solutions Team.';
        $email_id = 'elabui2016@gmail.com';
        if(mail($email_id, "Incredible Solutions - OTP Password Generation", $message, $headers))
            echo 'Email send successfully';
        else
            echo 'Something went wrong, try again!!!';
        exit;
    ...
    public function add_service_post() {
        ...
        if (!empty($data_input)) {
            if (!empty($data_input['service_type'])) {
                // $data["last_id"] = $this->ref_increment_model->get_increment_id('TKNO', 'TKNO');
                ...
                foreach ($attachment_list as $file_name => $file_list) {
                    if (!empty($_FILES[$file_name]['name'])) {
                        ...
                        for ($i = 0; $i < $filesCount; $i++) {
                            ...
                            // $uploadData[$i]['updated_date'] = date('Y-m-d H:i:s');
                ...
                foreach ($attachment_list as $file_name => $file_list) {
                    if (!empty($_FILES[$file_name]['name'])) {
                        ...
                        for ($i = 0; $i < $filesCount; $i++) {
                            ...
                            // $uploadData[$i]['updated_date'] = ($updated_date != '' ? $updated_date : date('Y-m-d H:i:s'));
