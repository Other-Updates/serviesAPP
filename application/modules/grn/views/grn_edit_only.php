    ...
    <div class="row">
                            ...
                            <table class="table table-striped table-bordered responsive dataTable no-footer dtr-inline"  id="add_quotation">
                                ...
                                <tbody id='app_table'>
                                    ...
                                    if (isset($po_details) && !empty($po_details)) {
                                        foreach ($po_details as $vals) {
                                            ...
                                            <tr class="inc_class<?php echo $vals['id'] ?>" data-inc="<?php echo $vals['id'] ?>">
                                                <td>
                                                    ...
                                                    <input type="hidden"  name="po_details_id[]" id="po_details_id" value="<?php echo $vals['po_details_id'] ?>"/>
                                                ...
                                                <td>
                                                    <div class="input-group">
                                                        <input class="form-control serial_number<?php echo $vals['po_details_id'] ?>" name="product_serial_no[]" readonly="">
                                                        <span class="input-group-append modal_show" style="display:none">
                                                            <label class="input-group-text" id="add_group" title="Add Serial Code" data-toggle="modal" data-target="#exampleModal-<?php echo $vals['po_details_id'] ?>" data-whatever="@mdo"><i class="fa fa-plus"></i></label>
                                                        </span>
                                        ...
                                        <div class="modal fade in" id="exampleModal-<?php echo $vals['po_details_id'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel-<?php echo $vals['po_details_id'] ?>" aria-hidden="false" align="center" style="display:none">
                                            <div class="modal-dialog">
                                                <div class="modal-content modalcontent-top">
                                                    <div class="modal-header modal-padding modalcolor">
                                                        <h5 class="modal-title">Insert Product S.No</h5>
                                                    ...
                                                    <div class="modal-body">
                                                        <form id="customer_model_form" action="<?php echo $this->config->item('base_url'); ?>" enctype="multipart/form-data" name="form" method="post">
                                                            <div id="append_div<?php echo $vals['po_details_id'] ?>">
                                                            </div>
                                                        </form>
                                                    </div>
                                                    <div class="modal-footer action-btn-align">
                                                        <button type="button" class="btn btn-primary btn-sm update_product_serial" id="update_product_serial">Update</button>
                                                        <button type="button" class="btn btn-danger btn-sm close_modal" id="no" data-dismiss="modal">Close</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
        ...
        <script>
            ...
            $(".deliver_qty").on('keyup', function () {
                ...
                var po_details_id = $(this).closest('tr').find('#po_details_id').val();
                ...
                if (Number(this_val) > Number(current_qty))
                {
                    ...
                    if (this_val) {
                        ...
                        append_serial_no(this_val, po_details_id);
                    } else {
                        $(this).closest('tr').find('span.modal_show').hide();
                    }
        ...
        <script type="text/javascript">
            ...
            function append_serial_no(count, id) {
                $("#append_div" + id).html("").html(new Array(++count).join('<div class="form-group row "><input type="hidden" class="po_det_id" value="' + id + '"> <label class="col-sm-3 col-form-label">Ser.No</label> <div class="col-sm-9"> <input type="text" class="form-control req_input">  <span class="error_msg"></span></div></div>'));
            }
            $('.update_product_serial').on('click', function () {
                var value_data = '';
                $(this).closest('div.modal').find('.req_input').each(function (index, value) {
                    value_data += $(this).val() + ',';
                });
                var get_po_details_id = $(this).closest('div.modal').find('.po_det_id').val();
                $('.serial_number' + get_po_details_id).val(value_data.slice(0, -1));
